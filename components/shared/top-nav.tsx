  ...
  return (
    <header className="fixed inset-x-0 top-0 z-40 border-b border-slate-200/80 bg-white/90 backdrop-blur">
      <div className="mx-auto flex h-16 w-full max-w-[1400px] items-center gap-3 px-4 md:px-6">
        <Link href={`/org/${orgSlug}/dashboard`} className="flex items-center gap-2 font-black tracking-tight text-slate-900">
          <span className="inline-flex h-8 w-8 items-center justify-center rounded-xl bg-blue-600 text-white">⚡</span>
          <span>TaskOS</span>
        </Link>

        <nav className="mx-auto hidden items-center gap-1 rounded-xl border border-slate-200 bg-white p-1 md:flex">
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={href}
                className={cn(
                  "rounded-lg px-3 py-1.5 text-sm font-semibold text-slate-600 transition-all duration-200 hover:bg-slate-100 hover:text-slate-900",
        ...
        <div className="ml-auto flex items-center gap-1">
          ...
          {canAccessAdminTools && (
            <Popover>
              ...
              <PopoverContent align="end" className="w-56 rounded-2xl border-slate-200 p-2">
          ...
          <Popover>
            ...
            <PopoverContent align="end" className="w-72 rounded-2xl border-slate-200 p-3">
